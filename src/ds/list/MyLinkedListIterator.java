...
public class MyLinkedListIterator<T> implements ListIterator<T> {
	private MyListNode<T> mLastRet;
	private MyListNode<T> mCurr;
	private int       mCusror;
	private final MyListIteratorCallback<T> mListIteratorCallback;
	
	public MyLinkedListIterator(MyListNode<T> head, MyListIteratorCallback<T> cb) {
		mCurr = head;
		mLastRet = null;
	...
	public MyListNode<T> previousNode() {
		if (!hasPrevious())
			throw new NoSuchElementException();
	...
	public void set(MyListNode<T> head) {
		mCurr = head;
		mLastRet = null;
	}
}
