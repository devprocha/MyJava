...
/*
 * A Thread safe "Bounded-Queue" based on a "Doubly-Linked List" implementation that  that "additionally" supports operations("blocking") that wait for the queue to become non-empty 
 * when retrieving an element, and wait for space to become available in the queue when storing an element.
    * 1) Algorithm : Linked List
...
public class LinkedBlockingQueueTest {
	...
	public static void Test() {	 
		LinkedBlockingQueue<String> bq = new LinkedBlockingQueue<String>(10); // Bounded queue with the size of 10
	...
	private static class ProducerThread implements Runnable {
		final LinkedBlockingQueue<String> mSharedQueue;
		final Thread mThread = new Thread(this);
		
		public ProducerThread (LinkedBlockingQueue<String> sharedQueue) {
	...
	private static class ConsumerThread implements Runnable {
		...
		public ConsumerThread (LinkedBlockingQueue<String> sharedQueue) {
			mSharedQueue = sharedQueue;
		}
